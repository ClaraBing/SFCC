condTree <- function(dtrain, dtest){
  cats <- levels(dtrain$Category)
  # train & predict
  var <- "~PdDistrict+X+Y+DayOfWeek+Year+Month+Day+HF"
  m <- ctree(as.formula(paste0("`", cats[1], "`", var)), data=dtrain[sample(200000),])
  ...
  for (i in 2:numCat){
    ...
    m <- ctree(as.formula(paste0("`", cats[i], "`", var)), data=dtrain[sample(200000),])
